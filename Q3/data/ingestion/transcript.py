...
class Speaker(BaseModel):
    ...
    id: str
    role: str = Field(..., pattern="^(sales|customer|other)$")
...
class TranscriptProcessor:
    ...
    def extract_features(self, transcript: Transcript) -> Dict:
        ...
        features = {
            ...
            "avg_confidence": sum(s.confidence for s in transcript.segments)
            / len(transcript.segments),
            "sales_turns": sum(
                1 for s in transcript.segments if s.speaker.role == "sales"
            ),
            "customer_turns": sum(
                1 for s in transcript.segments if s.speaker.role == "customer"
            ),
