...
class Word2VecClassifier:
    def __init__(self, categories: List[str], model_path: str = None):
        """
        Initialize Word2Vec classifier
        
        Args:
            categories: List of category names for classification
            model_path: Path to pre-trained Word2Vec model (optional)
        """
        self.embedder = Word2VecEmbedder(model_path)
    ...
    def predict(self, texts: Union[str, List[str]]) -> Dict[str, Any]:
        ...
        # Get the mapping of classifier's classes to indices
        class_to_idx = {class_: idx for idx, class_ in enumerate(self.classifier.classes_)}
